...
void agrupar_backtracking(int ind_ingreso, int ingreso[], int cant_ingresos, int capacidad[],
                          int cant_ubicaciones, vector<int> &rack) {
    // Caso base
    ...
    for (int ind_capacidad = 0; ind_capacidad < cant_ubicaciones; ind_capacidad++) {
        ...
        if (ingreso[ind_ingreso] + rack[ind_capacidad] <= capacidad[ind_capacidad]) {
            ...
            agrupar_backtracking(ind_ingreso + 1, ingreso, cant_ingresos, capacidad, cant_ubicaciones, rack);
...
int main() {
    ...
    agrupar_backtracking(0, ingreso, sizeof(ingreso)/sizeof(ingreso[0]), capacidad, rack.size(), rack);
    return 0;
}
